...
if(isset($_POST["user_first_name"]))
{
    	...
    	if(in_array($img_ext, $extensions) === true)
    	{
    		...
    		if(in_array($img_type, $types) === true)
    		{
    			if($image_size <= 2000000)
    			{
		    		...
		    		{
		    			$error .= '<li>Image dimension should be within 225 X 225</li>';
		    		}
	    		}
	    		else
	    		{
    ...
    {
    	$error .= '<li>Please Select Profile Image</li>';
    }
    ...
    if($error == '')
    {
        ...
        else
        {
        	...
        	$data = array(
        		...
        		':user_status'				=>	'Offline',
        		':user_datetime'			=>	$object->now,
        		':user_verification_code'	=>	$user_verification_code
        	);

        	$object->query = "
        	INSERT INTO user_cpmvj 
        	(user_first_name, user_last_name, user_email, user_password, user_image, user_status, user_datetime, user_verification_code) 
        	VALUES (:user_first_name, :user_last_name, :user_email, :user_password, :user_image, :user_status, :user_datetime, :user_verification_code)
        	";

        	$object->execute($data);

        	if($object->connect->lastInsertId() > 0)
        	{
        		$success_message = 'Registration Completed, now you can login into Chat System';
        	}
            else
            {
